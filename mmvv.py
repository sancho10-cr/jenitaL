...
def segment_image(filtered_array):
    ...
    # Filter out small objects to isolate the tumor better
    ...
    for region in regions:
        # Consider only regions with a significant volume (e.g., greater than 1000 voxels)
        if region.area > 1000:
...
def process_ct_image(file_path):
    ...
    return ct_image, ct_array, tumor_volume_cc, tumor_stage, metadata, labels, tumor_size, tumor_perimeter, tumor_area

# Function to display 3D image and results
def display_3d_image(ct_array, labels):
...
# Function to overlay the segmentation on original slices
def display_overlay(ct_array, labels):
    fig, axes = plt.subplots(3, 1, figsize=(10, 15))
    slice_indices = [ct_array.shape[0]//3, ct_array.shape[0]//2, 2*ct_array.shape[0]//3]
    
    for ax, slice_index in zip(axes, slice_indices):
        ax.imshow(ct_array[slice_index], cmap='gray')
        ax.imshow(labels[slice_index], cmap='Reds', alpha=0.5)
...
def select_file():
    ...
    if file_path:
        try:
            ct_image, ct_array, tumor_volume_cc, tumor_stage, metadata, labels, tumor_size, tumor_perimeter, tumor_area = process_ct_image(file_path)
            ...
            # Display 3D tumor visualization within the lung context
            display_3d_image(ct_array, labels)

            # Display overlay of segmentation on original slices
            display_overlay(ct_array, labels)
